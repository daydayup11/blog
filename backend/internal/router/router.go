...
func Setup(h Handlers, jwtSecret, uploadDir, frontendDir string) *gin.Engine {
	...
	api.GET("/works/sections", h.Garden.ListWorksSections)
	...
	admin.GET("/posts/:slug", h.Post.AdminGetBySlug)
	...
	admin.GET("/garden/sections", h.Garden.AdminListAllSections)
	admin.POST("/garden/sections", h.Garden.AdminCreateSection)
	admin.POST("/works/sections", h.Garden.AdminCreateWorksSection)
